"""lockatus_client.py — cliente OIDC mínimo para federar Anonimal con Lockatus (el hub de identidad
de la suite). Solo se usa si ANONIMAL_AUTH_MODE=federado. Stdlib (urllib) para no sumar httpx;
`cryptography` se importa LAZY dentro de verify_jwt → el modo local no lo necesita instalado.
Verifica los tokens RS256 contra el JWKS del hub (offline)."""
import base64
import hashlib
import hmac
import json
